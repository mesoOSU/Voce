...
void readfrmfile (double *y,double *x, int s, long t);
...
void main(int argc, char *argv[]) {
 ...
 for (t=3000; t <= atol(argv[1]); t+=saveT) {
        for (s=0;s<6;s++){
 ...
 sprintf(filename,"EVM_file_S%04d.txt",t);
 fp=fopen(filename,"w");
...
/*FILE *fp1;
 ...
 //sprintf(filename1,"SVM_hex_S%04d.txt",t);
...
void readfrmfile(double *y, double *x, int s, long t) {
  ...
  char filename[50000];
  //char filename1[1000];
  sprintf(filename,"eps_%d_%04d.txt",s+1,t);
  fp = fopen(filename,"r");
 //exit(0);
    for(i=0;i<MESHX;i++) {
        for(j =0;j<MESHY;j++) {
    ...
    /*sprintf(filename1,"sig_%d_S%04d.txt",s+1,t);
  fp1 = fopen(filename1,"r");

    for(i=0;i<MESHX;i++) {
        for(j =0;j<MESHY;j++) {
            for(z =0;z<MESHZ;z++){
                index = z + (j)*64 + (i)*64*64;
